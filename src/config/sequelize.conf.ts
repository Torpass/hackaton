...
const CategoryDB = sequelize.define("category", CategoryModel, {
  timestamps: true,
});
const DonationDB = sequelize.define<DonationInstance>(
  "donation",
  DonationModel,
  { timestamps: true }
);
const PatientDB = sequelize.define<PatientInstance>("patient", PatientModel, {
  timestamps: true,
});
...
const TreatmentDB = sequelize.define<TreatmentInstance>(
  "treatment",
  TreatmentModel,
  { timestamps: true }
);
...
const MedicationPathologyDB = sequelize.define(
  "medication_pathology",
  MedicationPathologyModel,
  { timestamps: true }
);
const MedicationExpirationDateDB =
  sequelize.define<MedicationExpirationInstance>(
    "medication_expiration_date",
    MedicationExpirationDateModel,
    { timestamps: true }
  );
const MedicationDisposalDB = sequelize.define(
  "medication_disposal",
  MedicationDisposalModel,
  { timestamps: true }
);
const MedicationDonationDB = sequelize.define<MedicationDonationInstance>(
  "medication_donation",
  MedicationDonationModel,
  { timestamps: true }
);
const DeliveryDB = sequelize.define<DeliveryInstance>(
  "delivery",
  DeliveryModel,
  { timestamps: true }
);
const DeliveryDetailsDB = sequelize.define(
  "delivery_details",
  DeliveryDetailsModel,
  { timestamps: true }
);
const ReturnDB = sequelize.define<ReturnInstance>("return", ReturnModel, {
  timestamps: true,
});
